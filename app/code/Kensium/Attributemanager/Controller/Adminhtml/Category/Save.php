    ...
    public function execute(){
        ...
        if ($data = $this->_request->getParams()) {
            if(empty($data['frontend_label'][1])){
                ...
                $this->_redirect($this->getUrl('attributemanager/category/edit', ['type' => $this->_type,'attribute_id' => $this->_request->getParam('attribute_id')]));
                return;
            }
            ...
            try {
                ...
                $this->_redirect($this->getUrl('attributemanager/category/index'));
                return;
            } catch (Exception $e) {
                $this->messageManager->addError($e->getMessage());
                $this->adminSession->setFormData($data);
                $this->_redirect($this->getUrl('attributemanager/category/edit'), array('type'=>$this->getRequest()->getParam('type'),'attribute_id' => $this->getRequest()->getParam('attribute_id')));
                return;
            }
        }
