        ...
        if ($data = $this->_request->getParams()) {
            if(empty($data['frontend_label'][1])){
                if($this->_request->getParam('attribute_id')){
                    ...
                    $this->_redirect($this->getUrl('attributemanager/customer/edit', ['type' => $this->_type,'attribute_id' => $this->_request->getParam('attribute_id')]));
                    return;
                }else{
                    $this->messageManager->addError('Label may not be empty');
                    $this->_redirect($this->getUrl('attributemanager/customer/edit', ['type' => $this->_type,'attribute_id' => 0]));
                    return;
                }
            }
            ...
            try {
                ...
                $this->_redirect($this->getUrl('attributemanager/customer/index'));
                return;
            } catch (Exception $e) {
                $this->messageManager->addError($e->getMessage());
                $this->adminSession->setFormData($data);
                $this->_redirect($this->getUrl('attributemanager/customer/edit'), array('type'=>$this->getRequest()->getParam('type'),'attribute_id' => $this->getRequest()->getParam('attribute_id')));
                return;
            }
        }
