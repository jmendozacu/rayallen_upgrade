...
namespace Kensium\TrackOrder\Helper;
...
class Guest extends \Magento\Sales\Helper\Guest
{
    ...
    )
    {
        ...
        parent::__construct(
            $context, $storeManager, $coreRegistry, $customerSession, $cookieManager, $cookieMetadataFactory, $messageManager, $orderFactory, $resultRedirectFactory
        );
    }
    ...
    public function loadValidOrder(App\RequestInterface $request)
    {
        ...
        if (empty($post) && !$fromCookie) {
        ...
        } elseif (!empty($post) && isset($post['oar_order_id'])) {

            $incrementId = $post['oar_order_id'];
            $zip = $post['oar_zip'];
            $storeId = $this->_storeManager->getStore()->getId();

            if (empty($incrementId) || empty($storeId) || empty($zip)) {
                $errors = true;
            }
            ...
            if ($order->getId()) {
                ...
                if (strtolower($zip) == strtolower($billingAddress->getPostcode())
                ) {
                    $errors = false;
                }
            }
        ...
        if (!$errors && $order->getId()) {
            if(!$order->hasShipments()){
                if(!($this->request->getActionName()=='invoice' || $this->request->getActionName()=='view' || $this->request->getActionName()=='print'))
                return $this->resultRedirectFactory->create()->setPath('trackorder/trackOrder/index/orderId/'.$order->getId());
                else
