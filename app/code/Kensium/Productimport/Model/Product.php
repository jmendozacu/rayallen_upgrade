 ...
 */
namespace Kensium\Productimport\Model;
...
class Product
{
    ...
    public function __construct(
        ...
        Product\ConfigConverter $configconverter,
    ...
    public function install(array $productFixtures, array $galleryFixtures)
    {
        ...
        foreach ($productFixtures as $fileName) {
            ...
            foreach ($rows as $row) {
                ...
                $attributeSetId = $this->catalogConfig->getAttributeSetId(4, $row['attribute_set_code']);
                $this->converter->setAttributeSetId($attributeSetId);
                if($row['product_type'] == 'configurable'){
                    $data = $this->configconverter->convertRow($row);
                    $pType = 'configurable';
                }else {
                    $pType = 'simple';
