 ...
 */
namespace Kensium\Review\Model;
...
class Review
{
  ...
  //  protected $productCollection;
    ...
    public function __construct(
        ...
        \Kensium\Review\Model\ResourceModel\Review $resourceReview,
    ...
    public function install(array $fixtures)
    {
        foreach ($fixtures as $fileName) {
            ...
            foreach ($rows as $row) {
                ...
                if (empty($productId)) {
                    $this->logger->info('Not existed:' . $row['sku']);
                    continue;
                }
                ...
                $dat=$dateArr[0];
                $month=$dateArr[1];
                $extractYearTime=explode(' ',$dateArr[2]);
                $year=$extractYearTime[0];
                $row['created_at']=$month.'/'.$dat.'/'.$year.' '.$extractYearTime[1];
                $row['created_at']=$this->date->date('Y-m-d H:i:s', $row['created_at']);
                $review1 = $this->reviewFactory->create();
                $review1->load($review->getId())->setCreatedAt($row['created_at']);
                $review1->save();
    ...
    {
        return $this->resourceReview->getProductIdBySku($sku);
