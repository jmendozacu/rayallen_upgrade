...
//     */
    ...
    public function createOrder($jsonOrderData, $customerAddress, $billingAddress, $shippingAddress,$items)
    {
        ...
        if(count($invalidType) > 0  || count($invalidSku) > 0 || count($invalidAmount) > 0 || count($invalidInventoryInfo) > 0){
            $errorMessage = '';
        ...
        if (!$customer->getEntityId() && $customerType == 'customer') {
            ...
            try {
                ...
                return ['error'=>FALSE,'msg'=> "Invalid customer address"];//$e->getMessage();
            }
