...
class Licensecheck {
    ...
    public function __construct(StoreManagerInterface $storeManagerInterface, resourceModelLicense $resourceModelLicense, ServerAddress $_serverAddress, Config $config, ResourceConnection $resource, DateTime $date, ScopeConfigInterface $scopeConfig, Session $adminSession, Lib\Common $common
    ) {
     ...
     * @param null $storeId
     */
    public function checkLicense($licenseKey, $from_cron, $storeId=NULL) {
        try {
            /**
                ...
                if ($serverIP != $ipAddress) {
                    ...
                    foreach ($allWebsites as $website) {
                        ...
                        foreach ($domains as $domain) {
                            if (strpos($getWebsiteUrl, 'http://') !== false && strpos($domain, 'http://') === false){
                                $domain = 'http://'.$domain;
                            }elseif(strpos($getWebsiteUrl, 'https://') !== false && strpos($domain, 'https://') === false){
                                $domain = 'https://'.$domain;
                            }
                        ...
                        foreach ($storeIds as $store_id) {
                            ...
                            if (in_array($storeCode, $storeViews)) {
                                ...
                                $requestedStoreIds[$store_id] = $storeCode;
                    ...
                    foreach ($domainsURLs as $domain) {
                        ...
                        if (in_array($domain, $allStoreDomainData)) {
                            ...
                            if (count($storeCodes) > 0) {
                                $storeViewCodeFlag = true;
                            }
                        }
                    }
                    if (!$storeViewCodeFlag) {
                        return 'Invalid Store Code|';
                    } else {
                        $storeCodeStr = '';
                        $websiteUrl = $website->getConfig('web/unsecure/base_url');
                        foreach ($requestedStoreIds as $keyStoreId => $storeCode){
                            $this->resourceModelLicense->licenseCheck($licenseKey, $status, $response, $keyStoreId, $websiteUrl);
                            $storeCodeStr .= $storeCode.',';
                            
                        }
                      return $status.'|'.$storeCodeStr;
                    }
              /*  } */
            }
        } catch (Exception $e) {
    ...
    public function getNewLicenseDetails() {
        ...
        $values .= base64_encode("License Type    : " . $this->_scopeConfig->getValue('license/add_domain_request/add_license_type')) . "\n";
        $values .= base64_encode("Email Recipient : " . $this->getNewAdminEmail());
        return $values;
    }
    ...
    public function getAdminEmail() {
        $emailId = $this->_scopeConfig->getValue('license/license_request/emailrecipient');
        if ($emailId == '') {
    ...
    public function getNewAdminEmail() {
        $emailId = $this->_scopeConfig->getValue('license/add_domain_request/add_email_recipient');
        if ($emailId == '') {
            return $this->_adminSession->getUser()->getEmail();
        } elseif ($emailId != '') {
            return $emailId;
