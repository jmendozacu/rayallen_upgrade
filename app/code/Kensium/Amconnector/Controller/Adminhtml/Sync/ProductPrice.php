...
class ProductPrice extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        \Kensium\Amconnector\Helper\ProductPrice $productPriceHelper,
...

    {
        ...
        $this->productHelper = $productPriceHelper;
    ...
    public function execute()
    {
        ...
        if ($backGroundSync) {
            ...
            if ($isLock) {
                $this->messageManager->addError("Product Price sync already running");
            } else {
                try {
                    exec("php " . BP . "/bin/magento kensium:sync productprice " . $syncId . " " . $gridSessionStoreId . " COMPLETE MANUAL NULL > /dev/null & 1 & echo $!", $out);
                    if (isset($out[0])) {
                        $pid = $out[0] - 1;
            ...
            $this->productHelper->priceSync('COMPLETE', 'MANUAL', $syncId, NULL, $gridSessionStoreId, NULL, NULL, NULL);
