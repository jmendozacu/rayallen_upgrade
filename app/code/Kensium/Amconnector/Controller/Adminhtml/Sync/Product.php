...
class Product extends \Magento\Backend\App\Action
{
     ...
     * @param \Kensium\Amconnector\Helper\Product $inventoryHelper
    ...
    public function execute()
    {
        ...
        if ($backGroundSync) {
            ...
            if ($isLock) {
                $this->messageManager->addError("Product sync already running");
            } else {
                try {
                    exec("php ".BP."/bin/magento kensium:sync product " . $syncId . " " . $gridSessionStoreId . " COMPLETE MANUAL NULL NULL> /dev/null & 1 & echo $!", $out);
                    if (isset($out[0])) {
                        $pid = $out[0] - 1;
            ...
            $this->productHelper->getProductSync('COMPLETE', 'MANUAL', $syncId, NULL, $gridSessionStoreId);
