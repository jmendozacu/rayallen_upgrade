define([
    ...
    $.widget('mage.customFormBuilder', {
        ...
        _create: function () {
            this.generateSubtypes();

            this.generateSaveEvent();
            var savedValue = $('[name="form_json"]').val();
            if (savedValue) {
                this.formData = window.JSON.parse(savedValue);
            }

            var formbId = 'frmb-' + $('ul[id^=frmb-]').length++,
            ...
            $('#amcustomform-element-tabs li:first').click();
        ...
        generateSaveEvent: function () {
            var self = this;
            $('#saveandcontinue, #save').click(function(){
                var form = document.getElementById(self.options.formID);
                var json = self.helpers.jsonSave(form);
                var formFields = ['input', 'textarea'];
                $('[name="form_json"]').val(json);

                $.each(formFields, function( key, value ) {
                    $('.form-wrap ' + value).removeAttr('required');
                });
            });
        },
        ...
        generateWrapperContent: function (formbId, $cbUL) {
            ...
            $('<div/>', {
                'class': 'help-notice',
                'html' : this.options.messages.helpNotice
            }).appendTo(this.stageWrap);
        ...
        fieldOptions: function (values) {
            ...
            if (!values.values || !values.values.length) {
                values.values = [1, 2, 3].map(function (index) {
                    var label = self.options.messages.option + ' ' + index;
                    var option = {
                        selected: false,
                        label: label,
            ...
            fieldOptions.push('<li><input type="radio" class="option-selected" value="false" name="'
                + values.name + '-option"></li>');

            this.utils.forEach(values.values, function (i) {
                fieldOptions.push(self.selectFieldOptions(values.name, values.values[i], isMultiple));
            });
        ...
        advFields: function (values) {
            var advFields = [],
                key,
                optionFields = ['dropdown', 'listbox', 'checkbox', 'radio', 'checkboxtwo', 'radiotwo'],
        ...
        observeFields: function ($cbUL, formbId) {
            ...
            this.sortableFields.on(
                'change blur keyup', '.form-elements input, .form-elements select, .form-elements textarea',
                self.saveAndUpdate()
            );
            ...
            this.sortableFields.on('change', '.prev-holder input, .prev-holder select', function (e) {
                ...
                if (self.utils.inArray(field.type, ['dropdown', 'listbox', 'checkbox', 'checkboxtwo', 'radio', 'radiotwo'])) {
            ...
            this.sortableFields.on('click touchstart', '.icon-copy', function (e) {
                ...
                self.helpers.updatePreview($clone);
                self.helpers.save();
            });
            ...
            this.sortableFields.on('click', '.add-opt', function (e) {
                ...
                $('.sortable-options', $optionWrap).append(self.selectFieldOptions(name, false, isMultiple));
            });
