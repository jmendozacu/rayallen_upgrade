...
echo "\n";
$fp = fopen(__DIR__ .'/googlefeed/rayallen.txt', 'w');
$i=0;
...
if ($_product->getTypeId() != 'configurable') {   
	fputcsv($fp, array(/*$result['gtin']*/ $product->getGtin(),$product->getSku(),$product->getSku(),$product->getName(),strip_tags($product->getDescription()),$catName,'https://www.rayallen.com/'.$product->getData('url_key'),'https://www.rayallen.com/pub/media/catalog/product'.$product->getData('image'),round($price,2).' USD',$color,$size,'in stock','New','unisex', $brand /*$result['brand'] ,$product->getBrand()*/,$gpcategory /*$result['google_product_category'], $product->getGoogleProductCategory()*/,'','','','','','','','','','','','Adult','','','','','','','',$cateFNames,'','','','','','','','','',''),"\t");
}
foreach ($associateproducts as $childProduct) {
